...
from collect.utils import CountedPaginator, paginate, tags_joiner, tags_splitter
...
def test_counted_paginator_does_not_count_the_objects():
    class Uncountable(list):
        def count(self, *args):  # pragma: no cover
            raise AssertionError("counted the objects again")

    paginator = CountedPaginator(Uncountable(range(10)), 4, count=10)

    assert paginator.count == 10
    assert paginator.num_pages == 3


def test_paginate_with_a_known_count():
    request = RequestFactory().get("/")

    page = paginate(request, qs=list(range(10)), limit=4, count=10)

    assert isinstance(page.paginator, CountedPaginator)
    assert page.paginator.count == 10
